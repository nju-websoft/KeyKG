...
public class PrunedBiHeap {
	...
	void AllPaths(){
		...
		for (int i = 0; i < keyNum; i++) {	//record all
			System.out.print(i + ":");
			for (int j : g1.invTable.get(realName[i]))
				System.out.print(" "+ j);
			System.out.println();
		}
		*/
	...
	double PrunedDP() {
		...
		for (int i = 0; i < keyNum; i++) {
			for (int it : g1.invTable.get(realName[i])) {
				CandiDTree newct = new CandiDTree(it, keyNum, i);
		...
		while (!Qplus.isEmpty()) {	//line 7
			CandiDTree ct = Qplus.poll();	//line 8
			if (ct.X.equals(P)) {	//line 9
				//System.out.println(best); 
				best = ct.weight;
				ansDtree = new DTreeInfo(ct.weight, ct.edges);
				//System.out.println(ct.v + " " + getloc(ct.X)  + " " + ct.weight + " " + ct.lb);	
				return keynode;
			}
			/*
			long currenttime = System.currentTimeMillis();
			if (currenttime - nowtime > 10*1000) {
				System.out.println((currenttime - startTime)/1000.0 + " s");
				nowtime = currenttime;
			}
			*/
			if (best <= appro * ct.lb - prec) return keynode;			
			...
			//if (ct.v == 10|| ct.v == 540530) System.out.println(ct.v + " " + getloc(ct.X)  + " " + ct.weight + " " + ct.lb);	
			BitSet Xbar = (BitSet)P.clone();			
			Xbar.xor(ct.X);
			int v = ct.v;	
	...
	//line 23-26
	void findSubset(CandiDTree ct, BitSet X, BitSet Xbar, int loc) {
		if (Xbar.cardinality() > 0) {
			DTree newdt = new DTree(ct.v, Xbar);
			if (Dplus.containsKey(newdt)) {
				if (ct.weight + Dplus.get(newdt).cost <= (best * 2 / 3) + prec) {
					X.xor(Xbar);
	...
	void update(int v, BitSet X, double cost, double lbhat, ArrayList<EdgeDP> edges, double tmpadd) {
		...
		//if (v == 10) System.out.println("gg" +v + " " + getloc(X)  + " " + cost + " " + lowb);	
		//there lowb==best can't be pruned, or right answer may not in Q 
		if (lowb - prec > best * appro) return;	//line 32
		...
		Qplus.push(new CandiDTree(v, X, cost, lowb, edges));		
	}
	...
	public double kdeal(GraphDP gg) {
		return kdeal(gg, 1.0, -1);
	}
	
	public double kdeal(GraphDP gg, double ap) {
		return kdeal(gg, ap, -1);
	}	
	
	public double kdeal(GraphDP gg, double ap, long cutTime) {
		...
		//ansDtree.print();
		return 0;
		//if (ansDtree == null) System.out.println("Noop! no tree"); else System.out.println(ansDtree.cost); 
	}
}
